...
三大核心模块(优先级从高到低):
1. **历史时间线 = 树形朝代脉络图**(git 风格树图,**从下到上**,朝代节点 +
   点击展开事件 + 人物,正确呈现三国 / 南北朝 / 五代十国并立)— **Phase 1 起点**
2. **历史人物卡 + 关键事件**(关系网、卡片浏览)
3. **数据热更**(app 二进制不变,数据从 GitHub/R2 拉取,push commit 即更新朋友的 app)

加分项(非核心,Phase 后期):
- **做题模块**(SRS 间隔重复)— 原核心模块,2026-05 降级,Phase 4+ 再考虑
- **人物对话**(LLM 扮演)— Phase 3+

不做:
- **反馈通道**(朋友吐槽走微信,app 不内置反馈入口)
...
- → **Phase 1**: **全朝代骨架(树图)+ β 深耕(尧舜禹+夏+商+西周)+ 数据热更**
  ...
  - 1.5 数据热更模块(详见 `docs/data-update-strategy.md`):
       ...
       hash 变 → 拉新数据 → 写本地 cache,下次启动用新数据
  - 1.6 在自己手机上能看的版本

  **工期估算**(2026-05 重估):
  - **全职 + AI 协助**: ~10-15 天(2-3 周冲刺)
  - **业余 + AI 协助**: ~1-2 个月
  - 1 周冲刺版可达"能玩"但 UI 粗糙、校对仓促,接受范围紧缩
  - 真正的瓶颈是**校对速度**(你阅读 LLM 起草内容)+ **Flutter 树图布局算法**

  **范围**:
  - 项目最终覆盖 **尧舜禹 → 改革开放前**(每朝代 ≤ 100 件最关键事件)
  - 时段终点: 1976/1978(改革开放前)
  - 朝代起讫年份**用夏商周断代工程为权威**(差异年份 yearUncertainty 标 century)
  - 朝代分支拓扑(三国/南北朝/五代十国 等并立)用 `regime` + `parentRegimeId`
