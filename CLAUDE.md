...
四大核心模块(优先级从高到低):
1. **历史时间线 = 树形朝代脉络图**(原"时间线"+"朝代脉络"已合并为单一模块,
   git 风格树图,**从下到上**,朝代节点 + 点击展开事件 + 人物,正确呈现三国
   / 南北朝 / 五代十国并立)— **Phase 1 起点**
2. **历史人物卡 + 关键事件**(关系网、卡片浏览)
3. **做题模块**(SRS 间隔重复,围绕事件 / 人物 / 朝代)
4. (原"朝代脉络图"已合并入 #1)
...
- → **Phase 1**: **全朝代骨架(树图)+ β 深耕(尧舜禹+夏+商+西周)**
  - 1.0 doc 重定位 v2 — 树图 UI + 全朝代范围(本次完成)
  - 1.1 数据源调研(多源 merge): Hellohistory + Wikidata + 维基百科年表
  - 1.2 全 25 朝代骨架: 朝代/政权/树拓扑(parentRegimeId / mergedIntoRegimeId),
       每朝代占位 5-10 条事件 + 5-10 个关键人物(只填 name + year/role)
